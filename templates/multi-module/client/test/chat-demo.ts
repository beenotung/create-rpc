import { getToken, onWSMessage, sendWsMessage } from '../src/api/utils'
import {
  getChatMessages,
...
async function switchRoom() {
  ...
  let json = await getChatMessages({ room_id: joinedRoomId })
...
sendButton.addEventListener('click', () => {
  ...
  let token = getToken()
  if (!token) {
    console.error('Not logged in. Chat requires token.')
    return
  }
  sendChatMessage({
